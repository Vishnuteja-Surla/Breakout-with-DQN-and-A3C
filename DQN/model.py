...
class AtariNet(nn.Module):
    ...
    def save_model(self, weights_file='saved_models/latest.pt'):
        if not os.path.exists("saved_models"):
            os.makedirs("saved_models")
        torch.save(self.state_dict(), weights_file)

    def load_model(self, weights_file='saved_models/latest.pt'):
        try:
            self.load_state_dict(torch.load(weights_file))
            print(f"SUCCESS: Loaded file {weights_file}")
        except:
